	...
	for (Tile* tile : tileList) {
		...
		if (CreatureVector* creatures = tile->getCreatures()) {
			...
			for (Creature* creature : *creatures) {
				...
				if (!params.isAggressive || (caster != creature && Combat::canDoCombat(caster, creature, params.isAggressive) == RETURNVALUE_NOERROR)) {
...
{
	bool canCombat = !params.isAggressive || (caster != target && Combat::canDoCombat(caster, target, params.isAggressive) == RETURNVALUE_NOERROR);
...
void Combat::doCombatDefault(Creature* caster, Creature* target, const CombatParams& params)
{
	...
	if (!params.isAggressive || (caster != target && Combat::canDoCombat(caster, target, params.isAggressive) == RETURNVALUE_NOERROR)) {
		SpectatorVec list;
